...
static var _fallback_service: DeadhandRNGService
...
static func generate_uid_static(prototype_data: PrototypeData) -> String:
	var service: DeadhandRNGService = _get_active_service()
...
static func _get_active_service() -> DeadhandRNGService:
	...
	if tree != null:
		...
		if node is DeadhandRNGService:
			return node
	if _fallback_service == null:
		_fallback_service = DeadhandRNGService.new()
...
func _emit_rng_rolled(track_name: String, result: Variant) -> void:
	...
	if bus != null and bus.has_signal("rng_rolled"):
		bus.emit_signal("rng_rolled", track_name, result)
